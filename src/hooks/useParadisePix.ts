...
export function useParadisePix(onApproved?: () => void) {
  ...
  const create = useCallback(async ({ amountCents, description, stage, customer }: CreateArgs) => {
    ...
    try {
      ...
      const { data, error: fnError } = await supabase.functions.invoke("kirvus-create-pix", {
      ...
      pollRef.current = window.setInterval(async () => {
        try {
          const r = await fetch(
            `${projectUrl}/functions/v1/kirvus-check-status?transaction_id=${encodeURIComponent(String(pixData.transaction_id))}`,
